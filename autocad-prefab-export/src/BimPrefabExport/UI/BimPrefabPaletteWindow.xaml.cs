...
public partial class BimPrefabPaletteWindow : Window
{
    ...
    private Guid? _autoSaveProductId;
    ...
    public BimPrefabPaletteWindow()
    {
        ...
        var materialCatalog = MaterialCatalogService.Default;
        BomGridSetup.ConfigureMaterialsGrid(MaterialsGrid, materialCatalog);
        BomGridSetup.ConfigureRebarsGrid(RebarsGrid, materialCatalog);
        MaterialsGrid.CellEditEnding += OnMaterialsGridCellEditEnding;
        RebarsGrid.CellEditEnding += OnRebarsGridCellEditEnding;
        BuildMainToolbar();
        BuildProductActionBar();
        BuildServerSyncPanel();
    ...
    private void RebuildServerDrawingRevisionsList(ProductPaletteRow pi)
    {
        if (ServerDrawingRevisionsList is null)
            return;

        ServerDrawingRevisionsList.Items.Clear();
        var revisions = pi.Record.ServerDrawingRevisions;
        if (revisions is not { Count: > 0 })
        {
            ServerDrawingRevisionsList.Items.Add("Henüz sunucuda kayıtlı çizim revizyonu yok.");
            return;
        }

        foreach (var rev in revisions)
        {
            var note = string.IsNullOrWhiteSpace(rev.ChangeNote) ? "" : $" · {rev.ChangeNote}";
            var when = string.IsNullOrWhiteSpace(rev.UpdatedAt) ? "" : $" · {rev.UpdatedAt}";
            ServerDrawingRevisionsList.Items.Add($"{rev.Revision} — {rev.Title}{note}{when}");
        }
    }

    private void ClearServerDrawingRevisionsList()
    {
        if (ServerDrawingRevisionsList is null)
            return;
        ServerDrawingRevisionsList.Items.Clear();
        ServerDrawingRevisionsList.Items.Add("Ürün seçildiğinde sunucu geçmişi burada görünür.");
    }
    ...
    private void WireAutoSaveHandlers()
    {
        ...
        {
            _autoSaveTimer.Stop();
            if (_autoSaveProductId is Guid id)
                TrySaveProductLocally(id, showErrors: false);
        };

        CodeBox.TextChanged += (_, _) => ScheduleAutoSave();
        QtyBox.TextChanged += (_, _) => ScheduleAutoSave();
        RevisionBox.TextChanged += (_, _) => ScheduleAutoSave();
        NoteBox.TextChanged += (_, _) => ScheduleAutoSave();
    }
    ...
    private void ScheduleAutoSave()
    {
        ...
        _autoSaveProductId = pi.Record.ProductId;
        _autoSaveTimer?.Stop();
    ...
    private void OnProductGridSelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        if (e.RemovedItems.Cast<object>().OfType<ProductPaletteRow>().FirstOrDefault() is ProductPaletteRow prev)
            TrySaveProductLocally(prev.Record.ProductId, showErrors: false);
    ...
    private void ApplySelectionToFields()
    {
        ...
        try
        {
            if (n == 1 && ProductGrid.SelectedItem is ProductPaletteRow pi)
            {
                ...
                RebuildPdfDrawingsList(pi);
                RebuildServerDrawingRevisionsList(pi);
            ...
            else if (n > 1)
            {
                ...
                ClearServerDrawingRevisionsList();
    ...
    private void LoadMaterialsForProduct(ProductRecord record)
    {
        ...
        foreach (var m in record.Materials ?? [])
        {
            ...
            if (string.IsNullOrWhiteSpace(line.MaterialCatalogCode)
                && !string.IsNullOrWhiteSpace(line.Code))
            {
                var byCode = MaterialCatalogService.Default.TryGetByCode(line.Code);
                if (byCode is not null)
                    line.MaterialCatalogCode = byCode.Code;
            }

            MaterialWeightHelper.NormalizeMaterialLine(line);
            _materials.Add(line);
        }
    }
    ...
    private void OnMaterialsGridCellEditEnding(object? sender, DataGridCellEditEndingEventArgs e)
    {
        if (e.Row.Item is not MaterialLine row)
            return;

        if (e.Column.Header as string == "Katalog")
            MaterialCatalogService.Default.ApplyCatalogSelection(row);

        MaterialWeightHelper.NormalizeMaterialLine(row);
    ...
    private void PopulatePaletteCategoryCombo()
    {
        ...
        PaletteCategoryCombo.ItemsSource = cat.Categories.ToList();
        UpdatePaletteCatalogHint();
    }

    internal void RefreshCatalogDropdowns()
    {
        PopulatePaletteCategoryCombo();
    ...
    private void UpdatePaletteCatalogHint()
    {
        if (PaletteCatalogHintText is null)
            return;
        PaletteCatalogHintText.Text = AttributeCatalogService.IsCatalogLoaded
            ? "Eleman tipi ve tipoloji PrecastFlow kataloğundan gelir."
            : "Eleman kimlik kataloğu PrecastFlow sunucusundan yüklenir. Tipoloji sekmesini kullanmak için giriş yapın.";
    }
    ...
    private void LoadPaletteTypology(ProductRecord record)
    {
        ...
        try
        {
            if (!AttributeCatalogService.IsCatalogLoaded)
            {
                ClearPaletteTypologyUi();
                UpdatePaletteCatalogHint();
                return;
    ...
    private void FlushPaletteTypologyToRecord(ProductRecord target)
    {
        ...
        if (PaletteCategoryCombo.SelectedItem is CategoryDefinition cat)
            target.ElementCategoryId = cat.Id;
        else if (!string.IsNullOrWhiteSpace(target.PrefabElementTypeId))
            target.ElementCategoryId = AttributeCatalogService.Default.GetCategoryIdForElementType(target.PrefabElementTypeId);
    ...
    private void OnNewProduct(object sender, RoutedEventArgs e)
    {
        ...
        var record = new ProductRecord
        {
            ...
            DisplayName = string.IsNullOrWhiteSpace(code) ? "Yeni ürün" : code,
            Code = code,
            Quantity = qty,
            Unit = "adet",
            Revision = 0,
    ...
    internal bool TrySaveCurrentProductLocally(bool showErrors)
    {
        if (ProductGrid.SelectedItems.Count != 1 || ProductGrid.SelectedItem is not ProductPaletteRow pi)
            return false;

        return TrySaveProductLocally(pi.Record.ProductId, showErrors);
    }

    private bool TrySaveProductLocally(Guid productId, bool showErrors)
    {
        if (_loading)
            return false;
        ...
        if (pi.Record.ProductId != productId)
            return false;
        ...
        using (var tr = doc.Database.TransactionManager.StartTransaction())
        {
            ...
            current.LocalUpdatedAtUtc = DateTime.UtcNow;
            current.SyncStatus = ProductDirtyTracker.IsDirty(current) ? "pending_push" : "synced";
            new RegistryService().SaveProduct(tr, doc.Database, current);
            tr.Commit();
        }

        pi.RefreshDisplay();
        SetStatus(ProductDirtyTracker.IsDirty(pi.Record) ? "Yerel kaydedildi (sunucuya gönderilmedi)." : "Yerel kaydedildi.");
        return true;
    }
    ...
    private void OnAddMaterialRow(object sender, RoutedEventArgs e)
    {
        ...
        _materials.Add(new MaterialLine { Category = "Diğer", Unit = "ad", Quantity = 1 });
    }
    ...
    private void OnAddRebarRow(object sender, RoutedEventArgs e)
    {
        ...
        RebarWeightHelper.NormalizeRebarRow(row, _rebars.Count + 1);
        _rebars.Add(row);
    }
    ...
    private void OnRemoveRebarRow(object sender, RoutedEventArgs e)
    {
        ...
        {
            _rebars.RemoveAt(i);
            for (var poz = 0; poz < _rebars.Count; poz++)
            {
                RebarWeightHelper.NormalizeRebarRow(_rebars[poz], poz + 1);
            }
